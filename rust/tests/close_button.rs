...
//! tiếp** lúc 12:25–12:29 để dọn sáu cửa sổ rỗng (`logs/huba.log`,
...
fn the_close_button_maps_to_the_close_route() {
    assert_eq!(
        huba::telegram::callback_to_command("close:win-ttys007").as_deref(),
        Some("/close win-ttys007")
    );
    assert_eq!(
        huba::telegram::callback_to_command("close:0a109818").as_deref(),
        Some("/close 0a109818")
    );
}
...
fn an_empty_close_payload_is_not_a_command() {
    assert_eq!(huba::telegram::callback_to_command("close:"), None);
}
...
fn open_and_close_are_different_buttons() {
    let open = huba::telegram::callback_to_command("sess:win-ttys003");
    let close = huba::telegram::callback_to_command("close:win-ttys003");
    assert_eq!(open.as_deref(), Some("/session win-ttys003"));
    assert_eq!(close.as_deref(), Some("/close win-ttys003"));
    assert_ne!(open, close);
}

fn bare_window(working: bool) -> huba::sessions::LiveSession {
...
fn an_idle_bare_window_closes_without_a_question() {
    assert!(!huba::sessions::closing_needs_confirm(&bare_window(false)));
}
...
fn a_busy_bare_window_closes_without_a_question_too() {
    assert!(!huba::sessions::closing_needs_confirm(&bare_window(true)));
}
...
fn a_cli_session_always_asks() {
    ...
    assert!(huba::sessions::closing_needs_confirm(&s));
}

/// Một hàng `kind = "shell"` mà lại CÓ tài khoản là thứ huba không dựng ra —
