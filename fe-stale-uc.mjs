...
// phải tắt hubd rồi chờ qua 5 phút"). Kịch bản này là nửa còn thiếu đó.
//
// Vì sao nó nợ tới hôm nay: đây là UC DUY NHẤT trong repo đòi TẮT chính cái nó
// đang kiểm — trong lúc `hubd` đứng im, hub MÙ với mọi phiên trên máy (không
...
// rồi coi là xong.
//
// NGƯỠNG CŨ ĐỌC TỪ SẢN PHẨM, KHÔNG ĐOÁN: `fe/index.html:1630-1631`
//   const ageMin = (Date.now() - Date.parse(snap.generated_at)) / 60000;
//   const stale = !(ageMin < 5);   // NaN (unparseable stamp) counts as stale
// và lý do chọn 5 phút, ngay phía trên nó (`fe/index.html:1628-1629`): "hubd
// cycles every ~120s (4s while following a session), so anything past 5
// minutes means it is not running." Chữ hiện ra khi cũ (`fe/index.html:1638-1641`):
...
//   2. Đứng im QUÁ 5 phút — không portal-push, không chạy `hub` tay. Trong lúc
//      này hub mù với mọi phiên; đó là cái giá của phép đo này.
...
//
// Nếu chạy khi ảnh chụp vẫn còn tươi (hubd chưa kịp tắt quá 5 phút), kịch bản
...
try {
  ...
  // do `fe-subagent-uc.mjs` tự đọc transcript thay vì tin `hub sessions --json`.
  ...
  // hành vi "bơm API thay UI" — trạng thái cũ/mới của ảnh chụp là do `hubd` có
  ...
  if (!snap || !snap.generated_at) {
    console.error("✗ CHƯA DỰNG ĐƯỢC TRẠNG THÁI CẦN ĐO: response /app/doc/list không mang snapshot (hub có thể CHƯA TỪNG portal-push).");
    console.error("  UC-S09 (nửa 'đã cũ') KHÔNG được tính là đạt bằng lần chạy này.");
    console.error("  Chạy `hub portal-push` một lần cho có ảnh chụp, rồi mới làm theo quy trình ở đầu file này.");
    await browser.close();
    process.exit(2);
  }
  ...
  if (ageMin < STALE_THRESHOLD_MIN) {
    console.error(`\n✗ CHƯA DỰNG ĐƯỢC TRẠNG THÁI CẦN ĐO: ảnh chụp còn TƯƠI (${ageMin.toFixed(1)} phút < ${STALE_THRESHOLD_MIN} phút) — hubd vẫn đang đẩy dữ liệu, hoặc mới tắt chưa đủ lâu.`);
    ...
    console.error(`    2) đứng im QUÁ ${STALE_THRESHOLD_MIN} phút — không portal-push, không chạy \`hub\` tay (hub sẽ mù trong lúc này, đó là giá của phép đo)`);
  ...
  check("chữ nói thẳng 'hub chưa đẩy dữ liệu mới'", /⚠ hub chưa đẩy dữ liệu mới/.test(stampText), stampText.slice(0, 90));
