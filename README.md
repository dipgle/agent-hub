...
> `CLAUDE.md`. Hệ quả thẳng thắn: **hub không còn tự tiêu tiền**; chỉ khi bạn
...
`/ask`, `/handover`, `/new`, `/tell` gọi `claude` thật nên **tốn tiền** — đúng
số tiền như bạn tự gõ ở terminal. Sổ `spend` ghi lại, im lặng; màn hình không
hiện giá.
...
| `spend` | mỗi lần gọi `claude` do bạn bấm, có giá — ghi để trả lời được, không để trưng |
