#!/usr/bin/env bash
# Create the self-signed code-signing identity that deploy/sign.sh uses.
#
# RUN THIS ONCE, EVER. The certificate IS the identity macOS remembers: every
# TCC grant given to hubd (Full Disk Access, Automation) is pinned to
...
# that no longer exists — hubd goes back to hanging at boot with no log. That is
...
echo "make-signing-cert.sh: identity ready — now run deploy/sign.sh"
