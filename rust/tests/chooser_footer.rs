...
//! environment?"* — huba thấy hộp (log `trust_dialog_not_this_box choices=3` từ
...
//! thì dòng `❯` duy nhất là **con trỏ hộp chọn** (`❯ 1. Set it up`). huba đọc
...
fn the_confirm_style_footer_counts_as_a_chooser() {
    assert!(
        huba::keys::has_chooser_footer(&dwork_screen()),
        "màn có hộp chọn mà cổng nói không — đây là màn đã làm huba mời bật auto mode"
    );
}
...
fn the_select_style_footer_still_counts() {
    let man = "❯ 1. Đổi tên file ở gốc repo\n  2. Chỉ sửa tham chiếu\n\
               Enter to select · ↑/↓ to navigate · Esc to cancel";
    assert!(huba::keys::has_chooser_footer(man));
}
...
fn two_scattered_phrases_are_not_a_footer() {
    ...
    assert!(!huba::keys::has_chooser_footer(prose));
}
...
fn the_chooser_cursor_is_never_read_as_typed_text() {
    ...
    let without_footer: String = s
        ...
        .join("\n");
    assert!(
        !huba::keys::has_chooser_footer(&without_footer),
    ...
    let only_cursor: String = without_footer
        ...
        .collect::<Vec<_>>()
        .join("\n");
    let read = huba::pipeline::prompt_line_text(&only_cursor);
    assert_eq!(
        read, None,
        "huba đọc con trỏ hộp chọn thành chữ trong ô nhập: {read:?} — cú `⏎ Gửi` \
...
fn real_typed_text_still_reads() {
    ...
    let without_footer: String = s
        ...
        .join("\n");
    assert_eq!(
        huba::pipeline::prompt_line_text(&without_footer).as_deref(),
        Some("làm việc 1, deploy dev rồi nghiệm thu UI"),
        "câu Hà gõ nằm ngay đó, huba phải đọc được"
    );
}
